...
if($from_if)
{
	...
	foreach($from_if as $from_if_element)
	{
		$from = $from_if_element;

		if($to_if)
		{
			foreach($to_if as $to_if_element)
			{
				$to = $to_if_element;

				if($from != $to)
				{
					if($iii > 0) $or_value = ' OR ';

					$where = $where.$or_value.'(bustrip.from='.$from.' AND bustrip.to='.$to.' AND bustrip.state = 1)';
