...
foreach ($order->passengers as $pass){

if( ($pass->passenger_status =="CONFIRMED") || ($pass->passenger_status_return =="CONFIRMED") ){
...
<table style="text-align: left;width:100%;margin:0;">
	<tr>
		<td style="border: none; width: 30%;">
			...
			<h3 align="center" style="margin-bottom: 0;">ООО "СКСавто"</h3>
			<h4 align="center" style="margin: 0 0 10px 0;">ИНН 7806523288</h4>
			<h3 align="center" style="color:red;">8(800)770-00-20</h3>
			</td>
		...
		if($pass->passenger_status == "CONFIRMED")
		{

		?>

			<?php echo $this->route->title ?>
		...
		if($pass->passenger_status_return == "CONFIRMED")
		{
		...
		<?php } ?>
				...
				<table cellpadding="0" cellspacing="0" width="100%">
					...
					foreach ($order->bustrips as $subject){ 
						...
						if( ( ($pass->passenger_status == "CONFIRMED") && ($ii==0) ) || ( ($pass->passenger_status_return == "CONFIRMED") && ($ii==1) ) )
						{

					?>

					<tr>
						<td class="nobold" style="border-top:1px solid #ddd;padding:4px 5px;">
...
echo JText::sprintf('COM_BOOKPRO_BUSTRIP_FROM_TXT',$subject->from_name,DateHelper::toShortDate($subject->depart_date),JFactory::getDate($subject->start_time)->format('H:i'),''); 
}
?></td>
...
if(strtotime($subject->end_time) < strtotime($subject->start_time)) {$correct_depart_date= DateHelper::toShortDate(strtotime($subject->depart_date) + (60*60*24));} else {$correct_depart_date=DateHelper::toShortDate($subject->depart_date);}

echo JText::sprintf('COM_BOOKPRO_BUSTRIP_FROM_TXT',$subject->to_name,$correct_depart_date,JFactory::getDate($subject->end_time)->format('H:i'),''); 
}
?>
				...
				<div>
					...
					<table class="table table-condensed" cellpadding="0" cellspacing="0" width="100%">
						<tr>
							...
							<?php }?>
							<?php if ( ($order->return_seat) && ($pass->passenger_status_return == "CONFIRMED") && ($pass->passenger_status == "CONFIRMED") ){?>
							<th style="border-top:1px solid #ddd;padding:4px 5px;">
						...
						<tr>
								...
								<?php if($pass->passenger_status == "CONFIRMED"){?>
								<td style="border-top:1px solid #ddd;padding:4px 5px;">
									<?php echo $pass->seat;?>
								...
								<?php if ( ($order->return_seat) && ($pass->passenger_status_return == "CONFIRMED") ){?>
								<td style="border-top:1px solid #ddd;padding:4px 5px;">
								...
								<td style="border-top:1px solid #ddd;padding:4px 5px;">
									...
									$pass_sum=0; $pass_sum1=0; $pass_sum2=0;
									if($pass->passenger_status == "CONFIRMED") $pass_sum1 = $pass->price + $pass->price_bag;
									if($pass->passenger_status_return == "CONFIRMED") $pass_sum2 = $pass->return_price + $pass->return_price_bag;

									echo CurrencyHelper::formatPrice($pass_sum1 +  $pass_sum2);	
...
*/ ?>
				...
				<?php if ($isModal) {
				 ...
				 ?>
				<div>
					<?php echo $config->get('invoice_footer'); ?>
				</div>
				<?php } ?>
