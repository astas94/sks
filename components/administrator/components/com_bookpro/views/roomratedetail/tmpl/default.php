...
if(!$from && !$to){
	...
	$to = date('Y-m-d',strtotime($todate.'14 day'));
            ...
            //List room
            $modelRoom = new BookProModelBusTrips();
...
<div class="span10">
 ...
 ?>
					...
					<tbody class="table-bordered">
						<tr>
							<th width="50px;"><?php echo $rooms[$j]->title; ?>
							</th>
							<?php  
							...
							?> <input type="text"
								...
								value="<?php echo $roomrest;?>" disabled="true" id="max_beds"
								...
								size="7" value="<?php echo $roomrate->number; ?>"
								id="<?php echo 'number'.$roomrate->id; ?>" name="number[]"> <br>
								<input type="text"
								style="width: 46px; height: 22px; text-align: center; margin: 1px 1px 1px 0px;"
								size="7" value="<?php echo $roomrate->rate; ?>"
								id="<?php echo "rate".$roomrate->id; ?>" name="rate[]"> <input
								type="hidden" name="id[]" value="<?php echo $roomrate->id; ?>">
								<input type="hidden" name="room_id[]"
								value="<?php echo $rooms[$j]->id; ?>"> <input type="hidden"
		...
		<div>
			<strong> <?php echo JText::_('COM_BOOKPRO_ROOMRATE_NOTE');?>
			</strong>:
			<?php echo JText::_('COM_BOOKPRO_ROOMRATE_NUMBER');?>
			<input type="text"
				style="border: 2px solid #53a853; width: 46px; height: 22px; text-align: center; margin: 1px 1px 1px 0px;"
				size="7"> ,
			<?php echo JText::_('COM_BOOKPRO_ROOMRATE_RATE');?>
			<input type="text"
				style="width: 46px; height: 22px; text-align: center; margin: 1px 1px 1px 0px;"
				size="7">
		</div>
		...
		<input type="hidden" name="option" value="<?php echo OPTION; ?>" /> <input
			...
			value="<?php echo CONTROLLER_ROOM_RATE_DETAIL; ?>" /> <input
