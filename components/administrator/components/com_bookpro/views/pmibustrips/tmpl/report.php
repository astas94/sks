	...
	<tr>
		...
		<td style="border: none; width: 50%; text-align: left;"><strong><?php echo $this->route->title ?><br />
			...
			<?php echo JText::_('COM_BOOKPRO_BUS').':'.(substr($this->route->bus_name,0,strpos($this->route->bus_name,'178')-10));
			    //echo substr($this->route->bus_name,0,strpos($this->route->bus_name,'178')-9);
			    //var_dump($this->route->bus_name);
			    //echo strlen($this->route->bus_name);
			?><br />
            Путевой лист №_________________<br />
            Водитель: ______________________<br/>
            <?php echo JText::_('COM_BOOKPRO_COMPANY').':'.$this->route->brandname;?>
			</strong
			></td>
	</tr>
...
<h3 align="center" style="margin: 0;"><?php 
    $stringbus='';
    if (strpos($this->route->code,'-') !== false){
        $stringbus=str_replace("-","",$this->route->code);
        if (strlen()>4){
        }
        else {
            $stringbus='0'.$stringbus;
        }
        
        if (strpos($this->route->code,'_') !== false){
            $stringbus=str_replace("_","",$stringbus);
            $stringbus=$stringbus.'2';
        }
        else {
            $stringbus=$stringbus.'1';
        }
    }
    else {
        $stringbus=$this->route->code;
        if (strlen()<=3){
            $stringbus='0'.$stringbus;
        }
        if (strpos($this->route->code,'_') !== false){
            $stringbus=str_replace("_","",$stringbus);
            $stringbus=$stringbus.'02';
        }
        else {
            $stringbus=$stringbus.'01';
        }
    }
    //echo ($stringbus);
    $stringnumber=JFactory::getDate($date)->format('Ymd');
    $stringnumber=$stringnumber.$stringbus;
    //echo($stringnumber);
    echo JText::_('ПОСАДОЧНАЯ ВЕДОМОСТЬ № ').$stringnumber; 
?></h3>
<?php 
//var_dump($this->items);
		   ...
		   // echo ("---------------".$itemsCount);
	        ...
	        <?php
    			...
    			//$temp_table_rows=array();
                    ...
                    $price_val_nform= number_format($subject->zprice,2);
		    $price_val= $subject->zprice;

    	            $sum_total=$sum_total+$price_val;$full_total=$full_total+$price_val;

			$temp_table_rows[]=array ('seat' => $subject->pseat, 'to' => $bustripto, 'type' => $subject->group_title, 'passport' => $subject->passport, 'name' => ($subject->lastname).' '.($subject->firstname).' '.$subject->midlename ,'price' => $price_val_nform, 'order_number' => $subject->order_number );
