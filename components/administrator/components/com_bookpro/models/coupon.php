  ...
  function getObjectByCode($code)
    {
    	...
    	$query->select ( '*');
    	$query->from('#__bookpro_coupon')->where('code='.$db->q($code))->where('total>0')
    	->where('publish_date<='.$db->q(JFactory::getDate()->toSql()))->where('unpublish_date>='.$db->q(JFactory::getDate()->toSql()));
    	$db->setQuery($query);
